...
const LOGO_BY_VERSION: Partial<Record<SimulatorVersion, string>> = {
  ...
  LEY_KARIN: '/assets/common/logos/icono-compass.svg',
...
const resolveMechanics = (config: SimulatorConfig | null): ResolvedMechanicConfig[] => {
  ...
  return config.mechanics.flatMap((mechanic) => {
    const registryEntry = MECHANIC_REGISTRY[mechanic.component_id ?? mechanic.mechanic_id];
    if (!registryEntry) {
...
export default function GestionEnSaludApp({
  ...
  const [contentPack, setContentPack] = useState<VersionContentPack>(initialContentPack);
  const [scenarioData, setScenarioData] = useState<ScenarioFile>(initialContentPack.scenarios);
  ...
  // init sounds
  useEffect(() => {
    upSoundRef.current = new Audio('/assets/common/sounds/indicator-up.mp3');
    downSoundRef.current = new Audio('/assets/common/sounds/indicator-down.mp3');
      ...
      if (selectedVersion === 'CESFAM') {
          ...
          if (week === 1) {
              ...
              setGameState(prev => ({
                  ...
                  eventsLog: [...prev.eventsLog, `Borrador semanal enviado en el día ${prev.day}.`],
              }));
          ...
          setGameState(prev => ({
              ...prev,
              lastScheduleSubmissionDay: prev.day,
              eventsLog: [...prev.eventsLog, `Planificación semanal enviada en el día ${prev.day}.`],
          }));
          setWarningPopupMessage('Planificación semanal enviada.');
          setActiveTab('interaction');
          return;
      }
    ...
    if (scenario) {
        ...
        if (option) {
            ...
            if (adminDecision) {
              ...
              const targetNpcId = adminDecision.targetRef.startsWith('stakeholder:')
                ? adminDecision.targetRef.replace('stakeholder:', '')
                : null;
              const targetNpc = targetNpcId ? gameState.stakeholders.find(stakeholder => stakeholder.id === targetNpcId) : null;
              mechanicEngine.emitCanonicalAction(
                'admin',
                adminDecision.actionType,
                adminDecision.targetRef,
                {
                  target_type: 'case_resolution',
                  target_id: targetNpcId,
                  target_label: targetNpc?.name ?? null,
                  day: gameState.day,
                  time_slot: gameState.timeSlot,
                  committed_day: gameState.day,
                  committed_time_slot: gameState.timeSlot,
                  source_node_id: scenario.node_id,
                  source_option_id: option.option_id,
                  summary: `Resolución administrativa: ${adminDecision.resolution}`,
                  mechanic_payload: {
                    resolution: adminDecision.resolution,
                    action_type: adminDecision.actionType,
                    target_ref: adminDecision.targetRef,
                    target_npc_id: targetNpcId,
                    target_npc_name: targetNpc?.name ?? null,
                    target_npc_role: targetNpc?.role ?? null,
                    source_node_id: scenario.node_id,
                    source_option_id: option.option_id,
  ...
  const renderMechanicTab = () => {
    ...
    const registryEntry = enabledEntry
      ? MECHANIC_REGISTRY[enabledEntry.component_id ?? enabledEntry.mechanic_id]
