...
import { buildMapVisitValueFinal } from '../services/mapActionExport';
...
const CesfamMap: React.FC<CesfamMapProps> = ({ gameState, onInteract }) => {
    ...
    const handleInteract = (staff: StaffMember, roomId: string) => {
        ...
        const previousValue = previousMapAction?.value_final && typeof previousMapAction.value_final === 'object'
            ? previousMapAction.value_final
            : {};
        const previousPayload = previousValue.mechanic_payload && typeof previousValue.mechanic_payload === 'object'
            ? previousValue.mechanic_payload
            : {};
        const originRoom = currentRoomRef.current
            ?? previousPayload.destination_room
            ?? previousValue.destination_room
            ?? 'director_office';
        const room = CESFAM_ROOMS.find(entry => entry.id === roomId);
