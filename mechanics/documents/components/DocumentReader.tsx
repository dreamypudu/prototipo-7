...
import { buildDocumentReadValueFinal } from '../services/documentActionExport';
...
const DocumentReader: React.FC<DocumentReaderProps> = ({ documents, readDocuments, onMarkAsRead }) => {
    ...
    const updateActiveScrollDepth = useCallback(() => {
        ...
        const depth = scrollableHeight <= 0
            ? 100
            : Math.min(100, ((element.scrollTop + element.clientHeight) / element.scrollHeight) * 100);
        active.maxScrollDepth = Math.max(active.maxScrollDepth, Math.round(depth * 10) / 10);
    }, []);

    const finishActiveDocumentView = useCallback(() => {
        const active = activeViewRef.current;
        if (!active) return;
        ...
        engine.emitCanonicalAction(
            'documents',
            'read_document',
            `doc:${active.document.id}`,
            buildDocumentReadValueFinal({
                document: active.document,
                openedAtMs: active.openedAtMs,
                closedAtMs,
                openedCount: active.openedCount,
                reopened: active.reopened,
                maxScrollDepth: active.maxScrollDepth,
                committedDay: active.committedDay,
                committedTimeSlot: active.committedTimeSlot,
                wasUnreadBeforeAction: active.wasUnreadBeforeAction,
            })
        );
        activeViewRef.current = null;
    }, [engine, updateActiveScrollDepth]);

    useEffect(() => () => {
        finishActiveDocumentView();
    }, [finishActiveDocumentView]);
    ...
    const handleSelectDoc = (doc: Document) => {
        if (activeViewRef.current?.document.id === doc.id) return;

        finishActiveDocumentView();
        setSelectedDocId(doc.id);
        const wasUnreadBeforeAction = !readDocuments.includes(doc.id);
        const openedAtMs = Date.now();
        const previousOpenEvents = gameState.mechanicEvents.filter(
            event => event.mechanic_id === 'documents' && event.event_type === 'open_document' && event.payload?.doc_id === doc.id
        ).length;
        const openedCount = Math.max(openedCountsRef.current[doc.id] ?? 0, previousOpenEvents) + 1;
        openedCountsRef.current[doc.id] = openedCount;
        activeViewRef.current = {
            document: doc,
            openedAtMs,
            openedCount,
            reopened: openedCount > 1 || !wasUnreadBeforeAction,
            maxScrollDepth: 0,
        ...
        engine.emitEvent('documents', 'open_document', {
            doc_id: doc.id,
            doc_title: doc.title,
            opened_count: openedCount,
            reopened: activeViewRef.current.reopened,
            day: gameState.day,
    ...
    return (
        <div className="bg-slate-950/70 p-6 rounded-xl border border-slate-800 animate-fade-in h-[75vh] flex flex-col shadow-[0_30px_80px_rgba(0,0,0,0.35)] backdrop-blur-md">
            ...
            <div className="flex-grow flex gap-5 overflow-hidden">
                ...
                <div ref={scrollContainerRef} onScroll={updateActiveScrollDepth} className="w-2/3 flex-grow overflow-y-auto">
                    {selectedDoc ? (
