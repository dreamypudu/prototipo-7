...
const CaseObjectivesPanel: React.FC<CaseObjectivesPanelProps> = ({
  ...
  return (
    <section className={`w-full rounded-2xl border border-white/12 bg-slate-900/85 shadow-[0_14px_32px_rgba(0,0,0,0.34)] ${className}`}>
      <button
        onClick={onToggle}
            ...
            {commitments.length === 0 ? (
              ...
              <ul className="space-y-2">
                {commitments.map((commitment) => (
                  <li key={commitment.expectedActionId} className="rounded-xl border border-white/10 bg-slate-950/60 p-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
